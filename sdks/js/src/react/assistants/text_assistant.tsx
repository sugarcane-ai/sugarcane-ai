  ...
  const upsertNudgeText = async (
    ...
    await textToAction(
      ...
      create ? "" : nudgeConfig.text,
      {
        ...currentPromptVariables,
        "#ACTION": action,
        "#IDLE_MESSAGE": currentNudgeConfig?.idle?.text,
        "#STUCK_MESSAGE": currentNudgeConfig?.stuck?.text,
        "#WELCOME_MESSAGE": currentNudgeConfig?.welcome?.text,
        "#EXIT_MESSAGE": currentNudgeConfig?.exit?.text,
        "#SUCCESS_MESSAGE": currentNudgeConfig?.success?.text,
      },
      newScope,
      true,
